...
# Make sure the sql table schema and order of the fields in the csv's match.
...
# Collects and stores the last words in the "street_name" string if they aren't in the expected list.
...
# Find tags that specify street names as k="addr:street".           
...
def audit(osmfile):
    """
        This returns a dictionary that matches the conditions of the function above.
    """
...
def is_postcode(elem):
    """checks to see if elem is a postcode"""
    return (elem.attrib['k'] == "addr:postcode" or elem.attrib['k'] == "postal_code")

def audit_postcode(postcodes, postcode):
    """ Gets a full list of entries related to postcode """
    postcodes[postcode].add(postcode)
    return postcodes

def update_postcode(postcode):
    """Creates a uniform format of 5 digit postcodes; Returns updated postcode"""
    if re.findall(r'^\d{5}$', postcode): # 5 digit postal codes.
        valid_postcode = postcode
        return valid_postcode
    elif re.findall(r'(^\d{5})-\d{4}$', postcode): # 9 digit postal codes
        valid_postcode = re.findall(r'(^\d{5})-\d{4}$', postcode)[0]
        return valid_postcode
    elif re.findall(r'MA\s*\d{5}', postcode): # postal codes with state abbreviations.
